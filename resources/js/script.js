/***********************
    ...
    // 1) sample services data (can add / change)
    const services = [
    ...
    // update add/remove button visibility based on cart
    ...
    function addToCart(id){
      ...
      if(!s) return;
      // avoid duplicates — assignment UI shows each item once
    ...
    // events for add/remove buttons (delegate)
    ...
    // hook for remove buttons on services (some may prefer remove in cart)
    // initial render
    renderServices();
    renderCart();

    // smooth scroll to booking section from hero button
